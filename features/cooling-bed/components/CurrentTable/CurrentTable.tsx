...
import { CurrentCoolingBed } from "../../schemas/cooling-bed";
...
export function CurrentTable() {
  const { data, isLoading } = useSuspenseQuery<CurrentCoolingBed[], Error>({
    queryKey: ["cooling-bed/current"],
    queryFn: async () => {
      const response = await fetch("/api/cooling-bed/current");
      return response.json();
    },
    refetchInterval: 30000,
  });

  const columnHelper = createColumnHelper<CurrentCoolingBed>();

  const columns: AccessorKeyColumnDefBase<CurrentCoolingBed, any>[] = useMemo(
    () => [
  ...
  return (
    <DataTable
      ...
      exportFileName={"current-cooling-bed"}
    />
  );
}
